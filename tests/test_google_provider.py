...
class GoogleProviderTests(unittest.TestCase):
    ...
    def test_requests_l16_and_writes_wav(self) -> None:
    ...
    def test_requests_and_writes_mp3_for_pro(self) -> None:
        calls: dict[str, object] = {}
        mp3 = b"ID3audio"
    ...
    def test_pro_requires_mp3_output(self) -> None:
        provider = GoogleProvider(
            api_key="secret", voice="Kore", client_factory=lambda **_: None
        )
    ...
    def test_rejects_an_unexpected_audio_format(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory, self.assertRaisesRegex(
            ProviderError, "after audio/l16 was requested"
        ):
            provider.synthesize(
                model="gemini-2.5-flash-preview-tts",
                document=parse("Пример."),
